...
export class WorkerController extends ApplicationController {
  ...
  register(app: express.Application, upload: multer.Instance): void {
    ...
    router.route("/all").get(this.getAllWorkers);
    router
      .route("/calendar-jobs")
      .get(
  ...
  async _create(
    ...
    res.status(202).json({ message: "Worker added successfully" });
  }
  ...
  async _update(
    ...
    // romove worker from old subcontractor
    const oldsubcontractor = ObjectId.isValid(user.subcontractorId)
        ? await Subcontractor.findOne(user.subcontractorId)
        : undefined;
      if (oldsubcontractor) {
        const newOldWorkerIds = oldsubcontractor.workerIds.filter((el) => el !== id);
        const oldSub = await this.subcontractor.findAndUpdate(
          user.subcontractorId,
          {
            workerIds: newOldWorkerIds,
          }
        );
      }
    ...
    if (attributes.subcontractorId) {
      ...
      if (subcontractor) {
        ...
        subcontractor.workerIds.push(String(user.id));
        await subcontractor.save();      
      }
    }
  ...
  async _index(
    ...
    if (status) {
      ...
      switch (status.toLowerCase()) {
        ...
        case "available":
          ...
          const minutes = moment(new Date()).format("mm");
          statusQ = {
            isActive: ACTIVE.active,
            isApproved: APPROVE.approved,
            workingHours: {
              $elemMatch: {
                "begin.hour": { $lte: hours },
                "begin.minute": { $lte: minutes },
                "end.hour": { $gte: hours },
                "end.minute": { $gte: minutes }
              }
            }
          };
          break;
    ...
    // const listUsers = await this.users.findAllNoPaginate(query);
  ...
  async addFcmToken(
    ...
    if (!user.roles.includes(EROLES.worker)) {
      res.status(403);
      res.send({
        message: "Only workers can add fcm tokens"
      });
    }
    ...
    if (!fcmToken) {
      res.status(400);
      res.send({
        message: "fcmToken is missed in body"
      });
    }

    const userToUpdate = await this.users.findById(user.id);
    if (!Array.isArray(userToUpdate.fcmToken)) {
      userToUpdate.fcmToken = new Array<string>();
    }

    if (userToUpdate.fcmToken.includes(fcmToken)) {
      res.status(200).send({
        message: "Nothing to update"
      });
    } else {
      const tokens =  [...userToUpdate.fcmToken, fcmToken];
      const updated = await this.users.customUpdate(userToUpdate, {fcmToken: tokens}, this.permitedAttributes);
      if (updated) {
        res.status(200).send({
          message: "Success"
        });
