...
export class SubcontractorController extends ApplicationController {
  ...
  async _update(
    ...
    const sub: any = await this.subcontractor.findById(req.params.id);
    const oldWorkerIds = sub.workerIds;

    const updateSub = await this.subcontractor.customUpdate(
      sub,
      attrs,
      this.permitedAttributes
    );
    ...
    if (updateSub) {
      for (let m = 0; m < oldWorkerIds.length; m++) {
        const subWorkerId = oldWorkerIds[m];
        const check = attrs.workerIds.includes(String(subWorkerId));
        if (check === false) {
          const updateWorker = await this.user.findAndUpdate(subWorkerId, {
      ...
      for (let i = 0; i < attrs.workerIds.length; i++) {
        const workerId = attrs.workerIds[i];
        const updateWorker = await this.user.findAndUpdate(workerId, {
          subcontractor: sub,
          subcontractorId: String(sub.id),
