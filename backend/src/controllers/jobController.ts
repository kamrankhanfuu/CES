...
import { JobRepositoryImpl, UserRepositoryImpl, } from "@repositories";
...
import { EROLES, JobStatus } from "@constants";
...
export class JobController extends ApplicationController {
  ...
  testSocket = async (req: Request, res: Response, next: NextFunction) => {
    ...
    await this.notificationService
        .sendJobNotification(NotifiableType.CREATE_JOB, {creatorId: req.user.id.toString(), job});
    ...
    for (const job of jobs) {
      ...
      job.workers = job.workers.map((item) => {
        if (
          ...
          const updated = {worker: {}, old: {}, new: {}} as any;
          const checkLocation = job.locations.filter((loca) => loca.address === location.address);
          ...
          const locationID = locationIndex > -1 ? locationIndex : job.locations.length + 1;
          item.status = 0;
          item.hasSeen = false;
          if (locationIndex < 0) {
            job.locations = [...job.locations, location];
          }
          updated.worker = item.worker;

          const notificationData = {creatorId: req.user.id.toString(), job, worker: updated};
          this.notificationService.sendJobNotification(NotifiableType.JOB_REROUTE_CURRENT, notificationData);

          return {...item, location, locationID } as JobWorker;
         }
        return item;
      });
      ...
      await fns.push(await this.job.customUpdate(job, new Job({...job}), this.permitedAttributes));
    }
  ...
  async _updateWorker(req: Request, res: Response, next: NextFunction) {
    ...
    const old = JSON.parse(JSON.stringify(job));
    job.setWorkerStatus(workerId, req.body.date, +req.body.status);
    old.notificationObj = job.notificationObj;
    if (old.notificationObj) {
      old.notificationObj.data.creatorId = req.user.id.toString();
    }
    this.job.author = req.user;
    await this.job.updateWithChangesLog(
      old,
      job,
  ...
  async _jobDuplicate(req: Request, res: Response, next: NextFunction) {
    ...
    const job = await this.job.findAllNoPaginate({ department: department });
  ...
  uploadWorkerImages = async (
    ...
    job.notificationObj = { type: NotifiableType.WORKER_UPLOAD_AN_IMAGE };
  ...
  async updateConfirmNumberJobs(
    ...
    const _jobs = await Promise.all(
      data.map(async (job: any) => {
        ...
        return await this.job.update(jobData, attrs, this.permitedAttributes);
      })
    );
  ...
  @rescuable
  async _addMany(req: Request, res: Response) {
    ...
    for (const job of data) {
      ...
      await this.job.update(jobData, attrs, this.permitedAttributes);

      if (old && Array.isArray(old)) {
        old.forEach((item) => {
          const notificationData = {creatorId: req.user.id.toString(), job, worker: item};
          this.notificationService.sendJobNotification(NotifiableType.JOB_REROUTE_NEW_JOB, notificationData);
        });
  ...
  async _findAll(req: Request, res: Response, next: NextFunction) {
    ...
    if (workerStatus) {
      ...
      req.query.workerStatus = workerStatus;
    }
    ...
    // }
      ...
      const found = await this.job.findAllWithPagination(req, query, isWorker, workerStatus);
  ...
  async _pdf(req: express.Request, res: express.Response, next: express.NextFunction) {
    const { params: { id } } = req;
    const job = await this.job.findOne(id.toString());
    if (!job) {
      res.status(404).send();
      return;
    }
    const file = await job.getPdfReport();
    res.download(file);
  }
...
//          job.workers[i].hasSeen = false  //apollo
  ...
  async _update(req: Request, res: Response, next: NextFunction) {
    ...
    if (Array.isArray(attrs.locations)) {
      const [location] = attrs.locations;
      job.workers.forEach(worker => worker.location = location);
    }
    ...
    this.job.author = req.user;
    job.notificationObj = {type: NotifiableType.EDIT_JOB, data: {creatorId: req.user.id.toString()}};
  ...
  async _updatePO(req: Request, res: Response, next: NextFunction) {
    ...
    for (let i = 0; i < req.body.ids.length; i++) {
      ...
      job.notificationObj = { type: NotifiableType.PO_NUMBER_HAS_BEEN_ADDED, data: {creatorId: req.user.id.toString()} };
      await this.job.update(job, { po: req.body.newPo });
    }
    res.sendStatus(202);
  }
