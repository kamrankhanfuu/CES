...
import { ACTIVE, APPROVE, DEPARTMENTS, EROLES, ROLES } from "@constants";
...
export class UserController extends ApplicationController {
  ...
  register(app: express.Application, upload: multer.Instance): void {
    ...
    router.route("/password-change")
      .put(passport.authenticate("jwt", { session: false }), this._passwordChange);
  ...
  async _passwordChange(
    ...
    if (!currentPassword || !password) {
      return res.status(400).send({
        message: "Bad request: currentPassword or password is missed"
      });
    }
    ...
    if (!user) {
      return res.status(400).send({
        message: "Unexpected error: invalid user"
      });
    }

    if (!user.roles.includes(EROLES.worker)) {
      return res.status(403).send({
        message: "Forbidden: only workers can change password in this way"
      });
    }
  ...
  async _activate(
    ...
    if (!user) {
      ...
      await this.users.activate(user);
