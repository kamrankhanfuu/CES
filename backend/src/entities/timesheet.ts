...
import { Job } from "./job";
import getMuniBoroughTimesheet from "../utils/getMuniBoroughTimesheet";
import {
  DEPARTMENT_GROUPS,
  DEPARTMENTS,
  flaggingPrices,
...
export class Timesheet extends ApplicationEntity {
  ...
  async beforeInsert() {
    ...
    this.requestDate = (await this.getRepository(Job).findOne(ObjectId(this.jobId))).requestTime;
  ...
  async loadJobParams() {
    ...
    if (job) {
      ...
      this.poet = job.poet;
      this.conEdisonSupervisorName = job.supervisorName;
  ...
  async getPdfReport(tz:any): Promise<string> {
    ...
    const params = {
      ...
      "requestDate": moment(this.requestDate).tz(tz).format("MM/DD/YYYY") || "",
      "requestTime": moment(this.requestDate).tz(tz).format("HH:mm ") || "",
      "requested": "",
      ...
      "poetNum": this.poet|| "",
      "conEdisonSupervisorName": this.conEdisonSupervisorName|| "",
      "workRequestNum":this.workRequest,
      "po": this.po|| "",
      "poet":this.poet|| "",
      "flagger": this.worker.name|| "",
      "confirmationNumber": this.confirmationNumber|| "",
      "startDate": moment(this.startDate).tz(tz).format("MM/DD/YYYY") || "",
      "startTime": moment(this.startDate).tz(tz).format("HH:mm ") || "",
      "lunch": this.hasLunch ? "x" : "  ",
      "dinner": this.hasDinner ? "x" : "  ",
      "noBreak": this.noBreak ? "x" : "  ",
      "finishDate": moment(this.finishDate).tz(tz).format("MM/DD/YYYY") || "",
      "finishTime": moment(this.finishDate).tz(tz).format("HH:mm ") || "",
      "totalHours": DateTime.getTimeDiff(this.finishDate, this.startDate)|| "",
      // "totalHours":this.totalHours || "",
      "conEdisonTruck": this.conEdisonTruck|| "",
      "conEdisonSupervisor": this.conEdisonSupervisorName || "",
      "receipt": this.receipt || "",
      "departmentName": this.departmentName|| "",
      "sign":this.sign,
      "muni1": getMuniBoroughTimesheet(this.locations.address.substring(this.locations.address.search(/\d{5}/),this.locations.address.search(/\d{5}/)+5)),
      "signerName" : this.signatureName || "",
      "employee" : this.employeeNumber || "",
      "date" : this.signedTime?moment(this.signedTime).tz(tz).format("MM/DD/YYYY") : "",
      "structure":this.locations?this.locations.structure : ""
    } as any;   
    params[`location1`] = this.locations.address;
    html = TemplateHelper.loadParams(html, params);    
    const options = {
      format: "Letter", "border": { left: "0.54cm", right: "0.54cm" },
      orientation: "landscape",
      height: "24.0cm", width: "40.94cm"
    };
  ...
  private realWorkTime() {
    let startDate: Date;
    let endDate: Date;
    if (new Date(this.startDate).getTime() > (this.requestDate ? new Date(this.requestDate).getTime() : 0)) {
