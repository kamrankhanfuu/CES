...
import { NotifiableType } from "./notification/NotifiableType";
...
export class Job extends ApplicationEntity {
  ...
  notificationObj: { type: number, data?: any };
  workerObjs: User[];
  ...
  workerView(worker: JobWorker) {
    const requestDateTime = new DateTime(
      moment(this.requestTime).toISOString(true)
    );
    const workerEndHr =
    ...
    const endTime =
      !workerEndHr || this.endTime
        ? new DateTime(
          moment(this.endTime).toISOString(true)
        )
        : new DateTime(
    ...
    return {
      ...
      onLocationTime: worker.onLocationTime,
      jobName: this.title,
      ...
      switch (status) {
        ...
        case WorkerJobStatus.ENROUTE:
          this.notificationObj = {
            type: NotifiableType.WORKER_EN_ROUTE,
            data: {worker: jobWorker}
          };
          // jobWorker.jobStatus = JobStatus.InProgress;
          break;
        case WorkerJobStatus.LOCATION:
          if (Date.now() > new Date(jobWorker.startDate).getTime()) {
            jobWorker.onLocationTime = new Date().toISOString();
          }
          this.notificationObj = {
            type: NotifiableType.WORKER_ON_LOCATION,
            data: {worker: jobWorker}
          };
          // jobWorker.jobStatus = JobStatus.InProgress;
          break;
        case WorkerJobStatus.SECURED:
          this.notificationObj = {
            type: NotifiableType.WORKER_SECURED_SITE,
            data: {worker: jobWorker}
          };
          // jobWorker.jobStatus = JobStatus.InProgress;
          break;
        case WorkerJobStatus.CANNOTSECURE:
          this.notificationObj = {
            type: NotifiableType.WORKER_CANNOT_SECURED_SITE,
            data: {worker: jobWorker}
          };
          // jobWorker.jobStatus = JobStatus.InProgress;
          break;
        case WorkerJobStatus.ENDOFSHIFT:
          this.notificationObj = {
            type: NotifiableType.WORKER_ENDED_SHIFT,
            data: {worker: jobWorker}
          };
  ...
  async getPdfReport(): Promise<string> {
    ...
    let html = fs.readFileSync(path.join(process.cwd(), "/src/templates/jobDetail.html"), "utf8");

    const reqDate = new DateTime(this.requestTime);
    let totals = {} as any;
    if (this.timesheets && this.timesheets.length) {
      
      totals = this.timesheets.reduce(
        (a: any, b: any) => {
          return {
    ...
    const params = {
      ...
      "departmentName": this.departmentName || "",
      "supervisorName": this.supervisorName || "",
      "municipality": this.municipality.label || "",
      "section": this.section || "",
      "confirmNumber": this.confirmNumber || "",
      "po": this.po || "",
      "feeder": this.feeder || "",
      ...
      "maxWorkers": this.maxWorkers || "",
      "receipt": this.receipt || "",
      "totalHours": totals.totalHours?totals.totalHours+"h":"0h",
      "regularHours": totals.regularHours?totals.regularHours+"h":"0h",
      "holidayHours": totals.holidayHours?totals.holidayHours+"h":"0h",
      "overtimeHours": totals.overtimeHours?totals.overtimeHours+"h":"0h",
      "billed": this.jobStatus || "",
      "comment": this.comment || "",
      "locations": this.locations || {},
      "workers": this.workerObjs || {},

    } as any;


    html = TemplateHelper.loadJobParams(html, params);
    let height = (this.locations.length/2)+(this.workerObjs.length/2)+18;
    const options = {
      format: "Letter", "border": { left: "2.54cm", right: "2.54cm" },
      orientation: "landscape",
      height: `${height || 20}cm`, width: "40.94cm"
    };
